    ...
    public class TopologicalMap
    {
        ...
        public bool AddOrUpdateState(State state)
        {
            ...
            if (_states.ContainsKey(state.Id))
            {
                ...
                _states[state.Id].VisitCount++;
                Console.WriteLine($"Stato '{state.Label}' (ID: {state.Id.Substring(0, Math.Min(10, state.Id.Length))}...) già presente. VisitCount incrementato a {_states[state.Id].VisitCount}.");
                return false;
            }
            else
            {
        ...
        public bool AddOrUpdateRule(Rule rule)
        {
            ...
            if (_rules.ContainsKey(rule.Id))
            {
                ...
                Console.WriteLine($"Regola '{rule.AppliedMIURule.Name}' (ID: {rule.Id.Substring(0, Math.Min(10, rule.Id.Length))}...) già presente. ApplicationCount incrementato a {_rules[rule.Id].ApplicationCount}.");
                return false;
            }
            else
            {
